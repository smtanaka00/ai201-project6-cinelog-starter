...
def add_to_watchlist(user_id, film_id):
    ...
    # Deduplication: scan for an existing (user_id, film_id) pair before
    # inserting so a repeat add is a clean domain error rather than a second
    # row (or a raw DB integrity error). Mirrors add_to_collection().
...
def get_watchlist(user_id):
    ...
    """
    # Sort newest-first (Comment 5). Matches get_collection() so the two list
    # features behave consistently rather than one being alphabetical.
    entries = (
